...
module.exports = async function handler(req, res) {
  ...
  const { name, phone, email, stage, pkg, suburb, notes, transmission, source } = req.body;
  ...
  const subjectLine = isContactForm
    ? `New Booking Request — ${name}`
    : `New Student Registration — ${name}`;
  const headerTitle = isContactForm ? 'New Booking Request' : 'New Student Registration';
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:20px">
          <div style="background:#E8650A;padding:18px 24px;border-radius:10px 10px 0 0">
            <h2 style="color:#fff;margin:0;font-size:18px">${escHtml(headerTitle)}</h2>
          ...
          <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px;padding:20px 24px">
            <table style="width:100%;border-collapse:collapse;font-size:14px">
              ...
              ${suburb ? `<tr><td style="padding:8px 0;color:#777;border-top:1px solid #f3f4f6">Suburb</td><td style="padding:8px 0;font-weight:600;color:#333;border-top:1px solid #f3f4f6">${escHtml(suburb)}</td></tr>` : ''}
              ${stage ? `<tr><td style="padding:8px 0;color:#777;border-top:1px solid #f3f4f6">Stage</td><td style="padding:8px 0;font-weight:600;color:#333;border-top:1px solid #f3f4f6">${escHtml(stage)}</td></tr>` : ''}
              ${pkg ? `<tr><td style="padding:8px 0;color:#777;border-top:1px solid #f3f4f6">Service</td><td style="padding:8px 0;font-weight:600;color:#333;border-top:1px solid #f3f4f6">${escHtml(pkg)}</td></tr>` : ''}
              ${transmission ? `<tr><td style="padding:8px 0;color:#777;border-top:1px solid #f3f4f6">Transmission</td><td style="padding:8px 0;font-weight:600;color:#333;border-top:1px solid #f3f4f6">${escHtml(transmission)}</td></tr>` : ''}
              ${notes ? `<tr><td style="padding:8px 0;color:#777;border-top:1px solid #f3f4f6;vertical-align:top">Notes</td><td style="padding:8px 0;font-weight:600;color:#333;border-top:1px solid #f3f4f6">${escHtml(notes)}</td></tr>` : ''}
            </table>
